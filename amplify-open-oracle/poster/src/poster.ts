...
export async function main(
  ...
  const payloads = await fetchPayloads(sources, web3, senderKey);
...
export async function fetchPayloads(sources: string[], web3: Web3, senderKey: string, fetchFn = fetch): Promise<OpenPriceFeedPayload[]> {
  ...
  return await Promise.all(sources.map(async (sourceRaw) => {
    ...
    try {
      if (typeof (source) === 'string') {
        ...
        response = await readCoinbasePayload(<CoinbaseConfig>source, web3, senderKey, fetchFn);
      }
